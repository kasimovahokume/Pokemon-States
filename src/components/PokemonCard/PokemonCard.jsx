...
export const PokemonCard =({pokemon,onAdd})=>{
  ...
  <div className={styles.content}>
    <img src={imgSrc} alt={pokemon.name} className={styles.image} />
    <h3 className={styles.name}>{pokemon.name}</h3>
    <p className={styles.type}>Type: {pokemon.type}</p>
    <div className={styles.addButton}>
       <ButtonCustom text="Add to Team" variant="add" onClick={onAdd} />
    </div>
  </div>
</div>
  )
